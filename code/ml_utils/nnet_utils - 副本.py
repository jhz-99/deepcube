...
def model_fn(features,labels,mode,args):
    ...
    nnet_layers.append(lambda x: layers_util.dense(x,5000,args.act_type,isTraining,doBatchNorm,args.l2,weightNorm,layerNorm,angleNorm))
    for layerIdx in range(0,args.num_l):
        nnet_layers.append(lambda x: layers_util.dense(x,args.num_h,args.act_type,isTraining,doBatchNorm,args.l2,weightNorm,layerNorm,angleNorm))
    for layerIdx in range(0,args.num_res):
        nnet_layers.append(lambda x: layers_util.resBlock(x,args.num_h,args.act_type,2,isTraining,doBatchNorm,args.l2,weightNorm,layerNorm,angleNorm))

    nnet_layers.append(dropoutSonnet)

    nnet_layers.append(lambda x: layers_util.dense(x,1,"linear",isTraining,False,args.l2,False,False,False))
